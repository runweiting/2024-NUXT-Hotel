...
import type { UserBooking } from '~/types/User'
...
export const useBooking = () => {
  ...
  const handleBooking = handleSubmit(async (values, { resetForm }): Promise<void> => {
    try {
      const payload: UserBooking = {
      ...
      // await bookingStore.xxxx(payload)
      resetForm()
    } catch (err) {
